        ...
        if(options.secure) {
          ...
          function handshake(data) {
            ...
            if ( headers["sec-websocket-key1"] && headers["sec-websocket-key2"] && upgradeHead ) { // 76
              ...
              var headerResponse = nano(handshakeTemplate76, {
                protocol: wsProtocol,
                resource: data.get[1],
                host:     data.host[1],
              ...
              socket.write(headerResponse, "binary");
              sys.puts(headerResponse)
              sys.puts("resource:" + data.get[1])
              sys.puts("  protocol:" + wsProtocol)
              sys.puts("  resource:" + data.get[1])
              sys.puts("  host:" +     data.host[1])
              sys.puts("  origin:" +   data.origin[1])
              //sys.puts("  data:" +     hash.digest("binary"))
