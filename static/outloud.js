...
var currentlyQueueingItem = null;
var favoritesopen = false;
var isqueueingId = null;
  ...
  $.getJSON('/' + roomname + '/listen/', {'c':cursor}, function(json){
    ...
    if(json.m instanceof Array){
      for(var i=0;i<=json.m.length;i++){
        processMessage($.parseJSON(json.m[i]), false);
      }
    }else{
     ...
     $.each(data.faves, function(key, message){
       ...
       if( 'scid' in message){
         newli.append($('<div class="fav_line scinfo"></div>')
             ...
             .append($('<span class="prevlink queue favqueue">queue</span>').data("tid", message['scid'])))
